...
parser.add_argument('--cfg', type=str, default='./output/08040309/train_v106_pl.yaml')
parser.add_argument('--diff_cfg', type=str, default='./output/08040309/train_v106.yaml', help='diff config file. like: two_spec_022401.yaml')
...
output_dir = f"{output_dir}/{cfg.general.suffix}"
os.makedirs(output_dir, exist_ok=True)
LOGGER = init_logger(f'{output_dir}/train_pl_{cur_time_abbr}.log')
...
LOGGER.info(f"num of available GPUs: {num_gpus}")
...
if os.path.exists(cfg.data.parquet_path):
    ...
    if cfg.data.use_33k:
        ...
        df_33k = df_33k[~(df_33k["response_a"]==df_33k["response_b"])] # filter out when repsonse a and response b are the same
...
if os.path.exists(cfg.data.pl_csv_path):
    pl_df = pd.read_csv(cfg.data.pl_csv_path)
    
    pl_df['id'] = pl_df['id'].astype(str) + "_pl"
    pl_df['prompt'] = pl_df['prompt'].apply(eval)
    pl_df['response_a'] = pl_df['response_a'].apply(eval)
    pl_df['response_b'] = pl_df['response_b'].apply(eval)

    pl_df = pl_df.sample(frac=0.5, random_state=cfg.general.seed).reset_index(drop=True)
    pl_df = pl_df[["id", "prompt", "response_a", "response_b", "winner_model_a", "winner_model_b", "winner_tie"]]

    df = pd.concat([df[["id", "prompt", "response_a", "response_b", "winner_model_a", "winner_model_b", "winner_tie"]], pl_df], axis=0)
    df = df.reset_index(drop=True)
...
tokenizer = GemmaTokenizerFast.from_pretrained(f"{cfg.model.model_name}")
...
def tokenize_cls_p3(example, tokenizer, max_length):
    ...
    for ps, ras, rbs in zip(example['prompt'], example['response_a'], example['response_b']):
        ...
        for idx, (p, ra, rb) in enumerate(zip(ps, ras, rbs)):
            r_tokens  = tokenizer(f'\n\n## Round {idx+1}:' if idx else f'## Round {idx+1}:', add_special_tokens=False)["input_ids"] # no changes in round 1
            ...
            # if current round all tokens sum is more than max_length
    ...
    labels = [[float(a_win), float(b_win), float(tie)] for a_win, b_win, tie in zip(example['winner_model_a'], example['winner_model_b'], example['winner_tie'])]
...
ds = ds.map(
    ...
    remove_columns=["id", "prompt", "response_a", "response_b", "winner_model_a", "winner_model_b", "winner_tie"],
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # KLDivLoss as our loss function
        loss_fct = nn.KLDivLoss(reduction='batchmean')

        # transfrom logits into log_softmax to make it compatible with KLDivLoss
