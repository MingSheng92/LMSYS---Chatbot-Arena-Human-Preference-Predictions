...
if os.path.exists(cfg.data.parquet_path):
    ...
    if cfg.data.use_55k:
        ...
        df_55k = df_55k[~((df_55k["response_a"]== '[null]') & (df_55k["response_b"]== '[null]'))] # remove when response_a and response_b both null
        df_55k = df_55k[~(df_55k["response_a"]==df_55k["response_b"])] # remove when both response the same
    ...
    if cfg.data.use_33k:
        ...
        df_33k = df_33k[~(df_33k["response_a"]==df_33k["response_b"])] # remove when both response the same
    ...
    for ps, ras, rbs in zip(example['prompt'], example['response_a'], example['response_b']):
        ...
        for idx, (p, ra, rb) in enumerate(zip(ps, ras, rbs)):
            ...
            if all_tokens_num > max_length:
                remain_tokens_num = max_length - prev_tokens_num - len(r_tokens) - 3*len(dot_tokens)  # remaining split p,a,b token
